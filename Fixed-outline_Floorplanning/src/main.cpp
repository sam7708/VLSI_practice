...
int Countwire(int degree, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4) {
	if (degree == 2)
		return abs(x1 - x2) + abs(y1 - y2);
	if (degree == 3)
		return max(max(x1, x2), x3) - min(min(x1, x2), x3) + max(max(y1, y2), y3) - min(min(y1, y2), y3);
	if (degree == 4)
		return max(max(max(x1, x2), x3), x4) - min(min(min(x1, x2), x3), x4) + max(max(max(y1, y2), y3), y4) - min(min(min(y1, y2), y3), y4);
	else
		return 0;
}
...
public:
	...
	TreeNode* GetInfo(int data, int &x, int &y, int &r, int &lc, int &ly);
	// Disturb function
...
TreeNode* BinaryTree::GetInfo(int data, int &x, int &y, int &r, int &lx, int &ly) {
	TreeNode *find = FindBlock(data);
	x = find->X;
	y = find->Y;
	r = find->R;
	lx = find->x_l;
...
int main(int argc, char** argv) {
	...
	idealX = sqrt(totalarea*atof(argv[5]));
	fin.close();
  ...
  start = clock();
	while (idealX < K.Max_x || idealX < K.Max_y) {
		int r = rand() % 15;
   stop = clock();
    if(double(stop - start) / CLOCKS_PER_SEC>3600){
       cout << K.Max_x << " " << K.Max_y << endl;
       break;
     }
		if (r <= PointA) {
	...
	for (int i = 0; i < nblock; i++) {
		int h, w, r, lx, ly;
		K.GetInfo(i, h, w, r, lx, ly);
		//cout << i << " " << h << " " << w << " " << r << " " << lx << " " << ly<<endl;
		Sb[i].h = h;
		Sb[i].w = w;
		Sb[i].R = r;
		Sb[i].llcx = lx;
	...
	while (fin.getline(ch, sizeof(ch), '\n')) {
     stop = clock();
		...
		if (ch[0] == 'p') {
			...
			dc--;
			x[dc] = Sb[StoI(ch)].llcx + Sb[StoI(ch)].w / 2;
			y[dc] = Sb[StoI(ch)].llcy + Sb[StoI(ch)].h / 2;
		}

		if (dc == 0) {
			Wirelength += Countwire(degree, x[0], y[0], x[1], y[1], x[2], y[2], x[3], y[3]);
		}
	...
	fp << "Wirelength " << Wirelength << endl;
	fp << "Blocks" << endl;
	for (int i = 0; i < nblock; i++) {
		fp << "sb"  << i << " "<<Sb[i].llcx << " " << Sb[i].llcy << " " << Sb[i].R << endl;
	}
	fp.close(); 
